 ...
 *
 * A compiled-and-linked FFI binding is a different situation: Rust's
 * `extern "C"` declarations need a *real*, externally-linkable symbol to
 * link against, and `static inline` functions don't produce one in the
 * compiled ctoon.c object. This file's only job is to give each of those
 * functions a thin, non-static wrapper with real (extern) linkage, so
 * they can be called across the Rust/C boundary — the wrappers themselves
