...
test "roundtrip nested" {
    ...
    try item_a.append(gpa, .{ .key = try gpa.dupe(u8, "sku"), .value = .{ .str = try gpa.dupe(u8, "A") } });
    try item_a.append(gpa, .{ .key = try gpa.dupe(u8, "qty"), .value = .{ .uint = 2 } });

    var item_b = try std.ArrayList(ctoon.Field).initCapacity(gpa, 2);
    try item_b.append(gpa, .{ .key = try gpa.dupe(u8, "sku"), .value = .{ .str = try gpa.dupe(u8, "B") } });
    try item_b.append(gpa, .{ .key = try gpa.dupe(u8, "qty"), .value = .{ .uint = 1 } });
    ...
    try order.append(gpa, .{ .key = try gpa.dupe(u8, "id"), .value = .{ .str = try gpa.dupe(u8, "ORD-1") } });
    try order.append(gpa, .{ .key = try gpa.dupe(u8, "items"), .value = .{ .array = items } });
    ...
    try root.append(gpa, .{ .key = try gpa.dupe(u8, "order"), .value = .{ .object = order } });
...
test "null and bool" {
    ...
    try fields.append(gpa, .{ .key = try gpa.dupe(u8, "a"), .value = .null });
    try fields.append(gpa, .{ .key = try gpa.dupe(u8, "b"), .value = .{ .boolean = true } });
    try fields.append(gpa, .{ .key = try gpa.dupe(u8, "c"), .value = .{ .boolean = false } });
...
test "negative and float" {
    ...
    try fields.append(gpa, .{ .key = try gpa.dupe(u8, "neg"), .value = .{ .sint = -42 } });
    try fields.append(gpa, .{ .key = try gpa.dupe(u8, "pi"), .value = .{ .real = 3.14 } });
